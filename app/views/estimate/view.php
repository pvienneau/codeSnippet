...
<div id="update_sidebar">
	...
	<?php if (!$estimate->invoiced): ?>
		<?php if ($estimate->status != 'accepted'): ?><li><a href="estimate/accepted/<?php echo $estimate->id; ?>" id="accepted"><?php echo __('Accepted');?></a></li><?php endif; ?>
		<?php if ($estimate->status != 'refused'): ?><li><a href="estimate/refused/<?php echo $estimate->id; ?>" id="refused"><?php echo __('Refused');?></a></li><?php endif; ?>
		<li><a href="#" id="send"><?php echo __('Send Estimate');?></a></li>
	<?php endif; ?>
		<li><a href="estimate/download/<?php echo $estimate->id; ?>" id="download"><?php echo __('Download PDF');?></a></li>
		<li><a href="#" id="history"><?php echo __('View History');?></a></li>
		<li><a href="public/estimate/<?php echo $estimate->permalink; ?>" class="blank" id="permalink"><?php echo __('Permalink');?></a></li>
		<li><a href="estimate/edit/<?php echo $estimate->id; ?>" id="edit"><?php echo __('Edit Estimate');?></a></li>
	<?php //if ($estimate->status != 'sent' && $estimate->status != 'accepted' && !$estimate->invoiced): ?>
		<li><a href="#" id="remove"><?php echo __('Remove Estimate');?></a></li>
	<?php //endif; ?>
	</ul>
</div>
<?php endif; ?>
...
<a href="#" class="add_more"><?php echo __('Edit email message');?>…</a>

<div class="control">
	<input type="submit" class="button" id="send_estimate" value="<?php echo __('Send');?>" />
...
<?php endforeach; ?>
		<li class="line invoice_line<?php if (!($index % 2)) echo ' alt'; ?>">
			...
			<span class="qty"><b><?php echo my_qty_format($total_hours).__('hour').($total_hours>1?'s':''); ?></b></span>
	...
	<div id="subtotal">
		...
		<?php if ($estimate->subtotal != $estimate->total): ?>
			...
			<?php if ($estimate->discount) echo "<p>".__('Discount')." ({$estimate->discount}%)</p>"; ?>
			...
			<?php if ($estimate->shipping) echo "<p>".__('Shipping')."</p>"; ?>
		<?php endif; ?>
...
<?php if (!$estimate->invoiced && AuthUser::accessLevel("estimates") > 1): ?>
	<a href="estimate/to_invoice/<?php echo $estimate->id; ?>" class="small_button"><?php echo __('Invoice Me');?></a>
<?php endif; ?>
